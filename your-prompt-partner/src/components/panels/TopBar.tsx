...
const TopBar = memo(() => {
  const { aircraft, satellites, earthquakes, vessels, toggleLeftPanel, mapMode, setMapMode, detectionMode, toggleDetectionMode, visualStyle, circularViewport, toggleCircularViewport, panopticEnabled, togglePanoptic, hudLayout, immersiveMode, warMode, toggleWarMode, triggerManualRefresh, newsLoading, droneMode, toggleDroneMode, epsteinMode, toggleEpsteinMode, setMapCenter, fullscreenMap, toggleFullscreenMap } = useWorldViewStore();
    ...
    >
      <div className="flex items-center gap-2.5">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <h1 className={`text-xs font-bold tracking-[0.25em] font-display ${warMode ? 'text-destructive glow-red' : 'text-foreground glow-green'}`}>WARMONITOR</h1>
        </div>
      ...
      <div className="flex items-center gap-2.5">
        ...
        <button
          ...
          className={`px-2 py-0.5 text-[8px] font-display tracking-[0.15em] rounded border transition-all ${
            fullscreenMap ? 'bg-primary/20 border-primary/40 text-primary' : 'border-primary/15 text-muted-foreground hover:text-primary hover:border-primary/30'
          }`}
          title={fullscreenMap ? 'Exit fullscreen' : 'Fullscreen map'}
        >
          {fullscreenMap ? '⊟ EXIT' : '⊞ FULL'}
        </button>
        ...
        <div className="relative" ref={keysRef}>
          <button
            ...
            className={`px-2 py-0.5 text-[8px] font-display tracking-[0.15em] rounded border transition-all ${
              keysOpen ? 'bg-primary/20 border-primary/40 text-primary' : 'border-primary/15 text-muted-foreground hover:text-primary hover:border-primary/30'
            }`}
          >
            ⌨ KEYS
          </button>
          {keysOpen && (
            <div className="absolute top-full right-0 mt-1 bg-background/90 backdrop-blur-sm border border-border/30 rounded p-2 text-[7px] font-data text-muted-foreground/60 space-y-0.5 animate-fade-in w-[260px] z-50">
              <div><span className="text-primary/60">1-8</span> Visual modes</div>
              ...
              <div><span className="text-primary/60">I</span> Immersive mode</div>
              <div><span className="text-primary/60">H</span> Cycle HUD layout</div>
