  ...
  return (
    <div className="absolute bottom-16 right-3 flex flex-col gap-1 pointer-events-auto z-30" style={{ maxHeight: 'calc(100vh - 200px)' }}>
      ...
      <div className="flex flex-col gap-0.5 bg-background/80 backdrop-blur-sm border border-border rounded p-1">
        <ControlBtn icon="+" title="Zoom In" onClick={() => {}} />
        <ControlBtn icon="−" title="Zoom Out" onClick={() => {}} />
        <div className="w-full h-px bg-border" />
        <ControlBtn icon="⟳" title="Reset View" onClick={() => flyTo(20, 0, 3)} />
        <ControlBtn icon={expanded ? '▾' : '▸'} title="Toggle Landmarks" onClick={() => setExpanded(!expanded)} />
      </div>

      {expanded && (
        <>
          {/* Category Tabs */}
          <div className="flex flex-wrap gap-0.5 bg-background/80 backdrop-blur-sm border border-border rounded p-1 mt-1 max-w-[140px]">
            {CATEGORIES.map(c => (
              <button
                key={c.key}
