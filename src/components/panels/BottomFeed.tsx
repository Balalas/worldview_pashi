...
const NewsCard = ({ item }: { item: NewsItem }) => {
  ...
  return (
    <div
      className={`bg-card-bg/60 border-l-2 ${severityColor} rounded-r px-2 py-1.5 hover:bg-card-hover transition-colors cursor-pointer`}
    ...
    <div className="h-full overflow-y-auto p-3">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-2">
        {PIZZA_INDEX_DATA.map((entry) => (
          <div
            key={entry.country}
            className={`bg-card-bg/60 rounded border p-2 ${
              entry.overUnder === 'over'
                ? 'border-alert-high/30'
                : entry.overUnder === 'base'
                ? 'border-primary/30'
                : 'border-alert-info/30'
            }`}
          >
            ...
            <div className="mt-1.5">
              ...
              <div className="text-[8px] font-data text-text-muted-custom mt-0.5">
                {entry.overUnder === 'over' ? `+${entry.index - 100}% overvalued` :
                 entry.overUnder === 'base' ? 'BASELINE' :
                 `${100 - entry.index}% undervalued`}
              </div>
            </div>
          </div>
        ))}
